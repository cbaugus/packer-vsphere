...
export TF_VAR_consul_acl_token=$(vault read -format=json consul/creds/elkstack-node | jq '.data.token' | tr -d '"')
...
export TF_VAR_remote_exec_ssh_key_file="/opt/devops-local/ssh/id_rsa"
